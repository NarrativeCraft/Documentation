...
Let's start by adding a zone, click on `Zones`, add a new one call it `zone1`, click the pen icon and click on `Place corners`.
`One time enter` means that the player enters the zone and the action is executed one time only
...
Now for a point. Open the points list, create a new one and name it `point1`, set the name stitch to `my_point`
